...
\begin{center}
\textbf{Some false narratives of the other papers that we need to debunk?}
\begin{itemize}
...
equal to row Parameters''. If this is true, then their model would technically be a 120M. The reason why we (and GPT2 paper) don't count the output matrix is because it is identical to the embedding matrix, this known as weight tying or weight sharing. Which does not seem the case with the other papers (I have contacted the other paper for clarifications through github).
	...
	\item  \textbf{Training data percentage}: both paper claim that they only used 3\% of the training data to train their models. We argue that limiting your training to only a fraction of the data does not correlate with the generalization capabilities of the compressed model, for the simple and following reasons:
		\begin{enumerate}
			...
			\item Dude, you literally use the output matrix as GPT2, have some shame.
		\end{enumerate}
\end{itemize}
