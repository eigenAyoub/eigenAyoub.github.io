...
Large Language Models (LLMs) are revolutionizing many industries and changing the way we perceive and interact with machines with (or thanks to) their rapid and unprecedented development. Their strong ability to understand, generate, and perhaps even "generalize" in numerous domains including automated text generation, language translation, and notably coding, has led many to believe that Artificial General Intelligence (AGI) is achievable with more scaling, a theory known as The Scaling Hypothesis \cite{branwen2021scaling}. Despite their advanced features, the core of LLMs still fundamentally relies on the initial Transformer architecture \cite{vaswani2017attention}.

The capabilities of their next-token prediction have exponentially increased at an unprecedented speed from GPT2 to the latest versions of GPT4, Claude3, etc.

While GPT-4 has fairly led the race in the past year, a lot of big actors have in the industry recently seems to catch up (e.g., Anthropic, Cohere, Google), and many believe that the two main ingredients are compute and data. And, while this seems affordable for big corps, it poses significant challenges for individuals and smaller organizations trying to replicate these advancements. And, advancing work on the compression and factorization of Large Language Models is essential to make these powerful tools more accessible and sustainable for broader use                                                           
